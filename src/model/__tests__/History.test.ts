...
describe(History.around.name, () => {
  ...
  test('around node with empty zone', () => {
    expect(around([1], 3))
      .toEqual({ items: [1], around: 1 })
    expect(around([2], 3))
      .toEqual({ items: [2], around: 2 })
    expect(around([4], 3))
      .toEqual({ items: [4], around: 4 })
    expect(around([5], 3))
      .toEqual({ items: [5], around: 5 })
    expect(around([2, 4], 3))
      .toEqual({ items: [2, 4], around: 4 })
    expect(around([1, 5], 3))
      .toEqual({ items: [1, 5], around: 5 })
  })
...
describe(History.insert.name, () => {
  ...
  test('insert in empty zones near items', () => {
    ...
    expect(insert([3], [1, 2]))
      .toEqual([1, 2, 3])
    expect(insert([3], [1, 2, 3]))
      .toEqual([1, 2, 3])
    expect(insert([3], [3, 4, 5]))
      .toEqual([3, 4, 5])
    expect(insert([3], [4, 5]))
      .toEqual([3, 4, 5])
    expect(insert([3], [5]))
  ...
  test('insert in empty zones near gaps', () => {
    ...
    expect(insert([3, [6, 7]], [4]))
      .toEqual([3, 4, [6, 7]])
    expect(insert([3, [6, 7]], [4], { down: false }))
      .toEqual([3, 4])
    expect(insert([3, [6, 7]], [5]))
      .toEqual([3, 5, [6, 7]])
    expect(insert([3, [6, 7]], [4, 5]))
      .toEqual([3, 4, 5, [6, 7]])
    expect(insert([3, [6, 7]], [5, 6]))
      .toEqual([3, 5, 6, [7, 7]])
    expect(insert([3, [6, 7]], [5, 6], { down: false }))
      .toEqual([3, 5, 6])
    expect(insert([3, [6, 7]], [5, 6, 7]))
      .toEqual([3, 5, 6, 7])

    expect(insert([[3, 4], 7], [6]))
      .toEqual([[3, 4], 6, 7])
    expect(insert([[3, 4], 7], [6], { up: false }))
      .toEqual([6, 7])
    expect(insert([[3, 4], 7], [5]))
      .toEqual([[3, 4], 5, 7])
    expect(insert([[3, 4], 7], [5, 6]))
      .toEqual([[3, 4], 5, 6, 7])
    expect(insert([[3, 4], 7], [4, 5]))
      .toEqual([[3, 3], 4, 5, 7])
    expect(insert([[3, 4], 7], [4, 5], { up: false }))
      .toEqual([4, 5, 7])
    expect(insert([[3, 4], 7], [3, 4, 5]))
      .toEqual([3, 4, 5, 7])
  })
})

describe(History.clearHistory.name, () => {
  const clearHistory = (history: SimpleHistory, upToId: number) => {
    const actualHistory = restoreHistory(history)
    History.clearHistory(actualHistory, upToId)
    return simplifyHistory(actualHistory)
  }

  test('clear empty history', () => {
    expect(clearHistory([], 0))
      .toEqual([])
    expect(clearHistory([], 10))
      .toEqual([])
  })

  test('clear only items', () => {
    expect(clearHistory([1, 2, 3, 4, 5], 0))
      .toEqual([1, 2, 3, 4, 5])
    expect(clearHistory([1, 2, 3, 4, 5], 2))
      .toEqual([3, 4, 5])
    expect(clearHistory([1, 2, 3, 4, 5], 4))
      .toEqual([5])
    expect(clearHistory([1, 2, 3, 4, 5], 5))
      .toEqual([])

    expect(clearHistory([3], 2))
      .toEqual([3])
    expect(clearHistory([3], 3))
      .toEqual([])
  })

  test('clear only gap', () => {
    expect(clearHistory([[1, 3]], 0))
      .toEqual([[1, 3]])
    expect(clearHistory([[1, 3]], 1))
      .toEqual([[2, 3]])
    expect(clearHistory([[1, 3]], 2))
      .toEqual([[3, 3]])
    expect(clearHistory([[1, 3]], 3))
      .toEqual([])

    expect(clearHistory([[5, 5]], 4))
      .toEqual([[5, 5]])
    expect(clearHistory([[5, 5]], 5))
      .toEqual([])
    expect(clearHistory([[5, 5]], 6))
      .toEqual([])
  })

  test('clear items followed by gap (trailing gap)', () => {
    expect(clearHistory([1, 2, [3, 7]], 0))
      .toEqual([1, 2, [3, 7]])
    expect(clearHistory([1, 2, [3, 7]], 1))
      .toEqual([2, [3, 7]])
    expect(clearHistory([1, 2, [3, 7]], 2))
      .toEqual([[3, 7]])
    expect(clearHistory([1, 2, [3, 7]], 3))
      .toEqual([[4, 7]])
    expect(clearHistory([1, 2, [3, 7]], 4))
      .toEqual([[5, 7]])
    expect(clearHistory([1, 2, [3, 7]], 7))
      .toEqual([])
    expect(clearHistory([1, 2, [3, 7]], 8))
      .toEqual([])
  })

  test('clear gap followed by items (leading gap)', () => {
    expect(clearHistory([[3, 7], 8, 9], 2))
      .toEqual([[3, 7], 8, 9])
    expect(clearHistory([[3, 7], 8, 9], 4))
      .toEqual([[5, 7], 8, 9])
    expect(clearHistory([[3, 7], 8, 9], 6))
      .toEqual([[7, 7], 8, 9])
    expect(clearHistory([[3, 7], 8, 9], 7))
      .toEqual([8, 9])
    expect(clearHistory([[3, 7], 8, 9], 10))
      .toEqual([])
  })

  test('clear in the middle: items, gap, items', () => {
    expect(clearHistory([1, 2, [3, 7], 8, 9], 0))
      .toEqual([1, 2, [3, 7], 8, 9])
    expect(clearHistory([1, 2, [3, 7], 8, 9], 1))
      .toEqual([2, [3, 7], 8, 9])
    expect(clearHistory([1, 2, [3, 7], 8, 9], 4))
      .toEqual([[5, 7], 8, 9])
    expect(clearHistory([1, 2, [3, 7], 8, 9], 7))
      .toEqual([8, 9])
    expect(clearHistory([1, 2, [3, 7], 8, 9], 9))
      .toEqual([])

    expect(clearHistory([4, [5, 5], 6], 3))
      .toEqual([4, [5, 5], 6])
    expect(clearHistory([4, [5, 5], 6], 4))
      .toEqual([[5, 5], 6])
    expect(clearHistory([4, [5, 5], 6], 5))
      .toEqual([6])
    expect(clearHistory([4, [5, 5], 6], 6))
      .toEqual([])
  })

  test('clear with multiple gaps around items', () => {
    expect(clearHistory([[1, 2], 3, [4, 5], 6, [7, 10]], 1))
      .toEqual([[2, 2], 3, [4, 5], 6, [7, 10]])
    expect(clearHistory([[1, 2], 3, [4, 5], 6, [7, 10]], 2))
      .toEqual([3, [4, 5], 6, [7, 10]])
    expect(clearHistory([[1, 2], 3, [4, 5], 6, [7, 10]], 3))
      .toEqual([[4, 5], 6, [7, 10]])
    expect(clearHistory([[1, 2], 3, [4, 5], 6, [7, 10]], 8))
      .toEqual([[9, 10]])
    expect(clearHistory([[1, 2], 3, [4, 5], 6, [7, 10]], 10))
      .toEqual([])
  })
})
