...
const localEmojies = require('./../json/localEmojies');
...
function hexToImg(hex, emoji) {
  if(localEmojies.find((code) => code == hex)) {
    return `<img class="emoji @${hex}" src="images/blank.gif" alt="${emoji}">`;
  }

  if(devicePixelRatio >= 2) hex += '_2x';
  return `<img class="emoji" src="https://vk.com/emoji/e/${hex}.png" alt="${emoji}">`;
}

function emojiReplace(emoji) {
  return hexToImg(emojiToHex(emoji), emoji);
}
